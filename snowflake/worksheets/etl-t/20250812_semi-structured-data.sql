...
-- flattening ingredients, is_dairy_free_flag, is_gluten_free_flag, is_healthy_flag, is_nut_free_flag, and menu_item_id
SELECT
    t2.value::string AS ingredients,
    t1.value:is_dairy_free_flag::string AS is_dairy_free_flag,
    t1.value:is_gluten_free_flag::string AS is_gluten_free_flag,
    t1.value:is_healthy_flag::string AS is_healthy_flag,
    t1.value:is_nut_free_flag::string AS is_nut_free_flag,
    menu_item_id
FROM
    menu,
    LATERAL FLATTEN(INPUT => parse_json(menu_item_health_metrics_obj):menu_item_health_metrics) AS t1,
    LATERAL FLATTEN(INPUT => t1.value:ingredients) AS t2;
