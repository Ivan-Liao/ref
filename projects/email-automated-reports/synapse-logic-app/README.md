   ...
   1. Source
      ...
      3. Use query (not table) — write the join and column selection directly in SQL, e.g.:
         ...
         SELECT
            ...
            f.Lot_Number                       AS "LotNumber",
            f.Flag_Bulk_Order                  AS "MultidoseOrderFlag",
            f.Flag_Redirected_Order            AS "RedirectedOrderFlag",
            f.Amount                           AS "DoseActivity",
            f.CalibrationDate                  AS "CalDate",
            f.CalibrationTime                  AS "CalTime",
            f.Order_Date                       AS "OrderDate",
            f.Order_Time                       AS "OrderTime",
            f.FilledDate                       AS "FilledDate",
            f.FilledTime                       AS "FilledTime",
            f.Order_Packed_Date                AS "PackedDate",
            f.Order_Packed_Time                AS "PackedTime",
            f.Order_Shipped_Date               AS "ShippedDate",
            f.Order_Shipped_Time               AS "ShippedTime",
            f.Order_Delivered_Date             AS "DeliveryDate",
            f.Order_Delivered_Time             AS "DeliveryTime"
         ...
         LEFT JOIN biwarp_biorx_mart.dbo.dim_procedures procedures
            ON f.Procedures_SID = procedures.Procedures_SID
         WHERE f.CalibrationDate >= CAST(DATEADD(day, -14, GETDATE()) AS DATE)
