1. Create instance
   ...
   2. Security >> allow full access to all Cloud APIs
2. SSH into instance
   1. Check instance specs `cat /proc/cpuinfo`
3. Install software
```
sudo apt-get update
...
wget https://storage.googleapis.com/spls/gsp008/requirements.txt
...
less ingest.sh (q to quit editor)
bash ingest.sh
```
5. Transform data
```
...
6. Create bucket
```
gcloud storage buckets create gs://qwiklabs-gcp-01-83c4633b6cba-bucket --project=$(gcloud config get-value project) --default-storage-class=standard --location=US --uniform-bucket-level-access

export BUCKET="gs://qwiklabs-gcp-01-83c4633b6cba-bucket"

gsutil cp earthquakes.* $BUCKET/earthquakes/

gcloud storage buckets add-iam-policy-binding $BUCKET --member=allUsers --role=roles/storage.objectViewer
gsutil acl ch -u AllUsers:R $BUCKET/earthquakes/earthquakes.csv
gsutil acl ch -u AllUsers:R $BUCKET/earthquakes/earthquakes.htm
gsutil acl ch -u AllUsers:R $BUCKET/earthquakes/earthquakes.png

https://storage.googleapis.com/qwiklabs-gcp-01-83c4633b6cba-bucket/earthquakes/earthquakes.png
```